...
export default function AdminUsers() {
  ...
  const fetchUsers = async () => {
    ...
    try {
      ...
      if (data.success) {
        ...
        toast({ title: 'Error', description: data.error || 'Failed to load users', variant: 'error' });
      }
    } catch (e: any) {
      console.error(e);
  ...
  const toggleStatus = async (id: string, currentStatus: boolean) => {
    ...
    try {
      ...
      if (data.success) {
        ...
        toast({ title: 'Error', description: data.error || 'Failed to update user', variant: 'error' });
      }
    } catch (e: any) {
      console.error(e);
      toast({ title: 'Error', description: e.message || 'Network error occurred', variant: 'error' });
    } finally {
      setProcessing(null);
    }
  };
  ...
  const deleteUser = async (id: string) => {
    ...
    try {
      ...
      if (data.success) {
        ...
        toast({ title: 'Error', description: data.error || 'Failed to delete user', variant: 'error' });
      }
    } catch (e: any) {
      console.error(e);
      toast({ title: 'Error', description: e.message || 'Network error occurred', variant: 'error' });
  ...
  const handlePasswordReset = async (e: React.FormEvent) => {
    ...
    if (newPassword.length < 8) {
      ...
      toast({ title: 'Error', description: 'Password must be at least 8 characters long', variant: 'error' });
      return;
    }

    setPasswordError('');
    setProcessing(resettingUser.id);
    try {
      ...
      if (data.success) {
        toast({
          ...
          description: `Password for ${resettingUser.email} has been updated.`,
          variant: 'success',
        });
        ...
        toast({ title: 'Error', description: data.error || 'Failed to reset password', variant: 'error' });
      }
    } catch (e: any) {
      console.error(e);
      toast({ title: 'Error', description: e.message || 'Network error occurred', variant: 'error' });
        ...
        <div className="p-4 border-b border-gray-200 flex flex-col sm:flex-row gap-4 items-center justify-between bg-gray-50/50">
          ...
          <div className="flex gap-3 w-full sm:w-auto">
            ...
            >
              ...
              <option value={UserRole.YOUTH_PRESIDENT}>Youth President</option>
              <option value={UserRole.HR_OFFICER}>HR Officer</option>
              <option value={UserRole.CHIEF_STAFF}>Chief Staff</option>
